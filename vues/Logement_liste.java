	...
	public class Logement_liste{
		...
		public static void main(String[] args) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						Logement_liste window = new Logement_liste();
		...
		private void initialize() {
			...
			cRecherche.setModel(new DefaultComboBoxModel(new String[] {"VILLE", "PRIX", "TAILLE", "ETAGE", "ID_PROPRIETAIRE", "ID_CATEGORIE"}));
			...
			comboagence.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					Logement_add add = new Logement_add();
			...
			comboagence_1.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					Logement_liste cat = new Logement_liste();
			...
			panel_1_1.setBounds(720, 573, 176, 27);
			panel.add(panel_1_1);
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(table.getSelectedRow()!=-1) {
						 ...
						 if(input==0) {
								...
								logedao.deleteById(selectedId);
								((DefaultTableModel)table.getModel()).removeRow(test);
								JOptionPane.showMessageDialog(null, "Le Logement bien été supprimé ");
						 }
			...
			panel_1.setBounds(379, 573, 176, 27);
			panel.add(panel_1);
			panel_1.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if(table.getSelectedRow()!=-1) {
						...
						LogementDAO logedao=new LogementDAO();
						Logement p=logedao.getById(selectedId);

								new Logement_edit(p).getFrame().setVisible(true);
			...
			panel_1_2.setBounds(47, 573, 176, 27);
			panel.add(panel_1_2);
			panel_1_2.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					Document doc =new Document();
								...
								PdfWriter.getInstance(doc, new FileOutputStream("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\loge.pdf"));
								doc.open();
								//ENTETE DEBUT
								...
								doc.add(new Paragraph("LOGEMENT"));
								Image img = Image.getInstance("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\immobilier.png");
								...
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								doc.add(new Paragraph("Liste des logements:"));
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								...
								cell = new PdfPCell(new Phrase("Taille",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								
								cell = new PdfPCell(new Phrase("Etage",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase("id_Categorie",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("ville").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("taille").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								
								cell = new PdfPCell(new Phrase (resultat.getString("etage").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("id_Categorie").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								doc.close();
												...
												try {
													Desktop.getDesktop().open(new File("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\loge.pdf"));
			...
			final JLabel lblNewLabel = new JLabel("Logement liste");
			lblNewLabel.setBounds(262, 0, 323, 56);
			...
			lblNewLabel_1.setIcon(new ImageIcon(Logement_liste.class.getResource("/res/room3.png")));
			lblNewLabel_1.setBounds(614, 11, 62, 48);
		...
		public static void remp() {
			LogementDAO logedao=new LogementDAO();
			 ...
			 String columns[] = { "ID", "VILLE", "PRIX", "TAILLE", "ETAGE", "ID_PROPRIETAIRE", "ID_CATEGORIE"};
		      String data[][] = new String[loge.size()][columns.length];

		      for(int i=0;i<loge.size();i++) {
		    	  data[i][0] = loge.get(i).getId()+"";
			        data[i][1] = loge.get(i).getVille();
			        data[i][2] = loge.get(i).getPrix()+"";
			        data[i][3] = loge.get(i).getTaille()+"";
			        data[i][4] = loge.get(i).getEtage()+"";
			        data[i][5] = loge.get(i).getId_Proprietaire()+"";
			        data[i][6] = loge.get(i).getId_Categorie()+"";
		...
		public static void recherche(String rech,String nom) {
			LogementDAO logedao=new LogementDAO();
			 ...
			 ArrayList<Logement> loge = logedao.Recherche(rech,nom);
		...
		public Window getFrameListeLogement() {
