...
import javax.swing.ImageIcon;
	...
	public class Proprietaire_liste{
		...
		public static void main(String[] args) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						Proprietaire_liste window = new Proprietaire_liste();
		...
		private void initialize() {
			...
			cRecherche.setModel(new DefaultComboBoxModel(new String[] {"NOM", "PRENOM", "VILLE", "DATEP"}));
			...
			comboagence.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
						Proprietaire_add add = new Proprietaire_add();
			...
			comboagence_1.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					Proprietaire_liste cat = new Proprietaire_liste();
			...
			final JLabel lblNewLabel = new JLabel("Proprietaire liste");
			lblNewLabel.setBounds(264, 0, 322, 55);
			...
			lblNewLabel_1.setIcon(new ImageIcon(Proprietaire_liste.class.getResource("/res/house.png")));
			lblNewLabel_1.setBounds(609, 0, 94, 66);
			panel.add(lblNewLabel_1);
			
			JPanel panel_1 = new JPanel();
			panel_1.setBounds(352, 577, 176, 27);
			panel.add(panel_1);
			panel_1.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if(table.getSelectedRow()!=-1) {
						...
						Proprietaire p=propdao.getById(selectedId);

								new Proprietaire_edit(p).getFrame().setVisible(true);
			...
			panel_1_2.setBounds(41, 577, 176, 27);
			panel.add(panel_1_2);
			panel_1_2.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					Document doc =new Document();
								...
								PdfWriter.getInstance(doc, new FileOutputStream("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\propr.pdf"));
								doc.open();
								//ENTETE DEBUT
								...
								doc.add(new Paragraph("PROPRIETAIRE"));
								Image img = Image.getInstance("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\proprietaire.png");
								...
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								doc.add(new Paragraph("Liste des propriétaires :"));
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								...
								doc.close();
												...
												try {
													Desktop.getDesktop().open(new File("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\propr.pdf"));
			...
			panel_1_1.setBounds(718, 577, 176, 27);
			panel.add(panel_1_1);
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(table.getSelectedRow()!=-1) {
						 ...
						 if(input==0) {
							 	int test=table.getSelectedRow();
								int selectedId = Integer.parseInt(table.getModel().getValueAt(test, 0).toString());
								propridao.deleteById(selectedId);
								((DefaultTableModel)table.getModel()).removeRow(test);
								JOptionPane.showMessageDialog(null, "Le proprietaire bien été supprimé ");
						 }
		...
		public static void remp() {
			ProprietaireDAO propridao=new ProprietaireDAO();
			 ...
			 String columns[] = {"ID", "NOM", "PRENOM", "VILLE", "DATEP"};
		      String data[][] = new String[prop.size()][columns.length];
		      
		      for(int i=0;i<prop.size();i++) {
		    	  data[i][0] = prop.get(i).getId()+"";
			        data[i][1] = prop.get(i).getNom();
			        data[i][2] = prop.get(i).getPrenom();
			        data[i][3] = prop.get(i).getVille();
			        data[i][4] = prop.get(i).getDateP();
		...
		public static void recherche(String rech,String nom) {
			ProprietaireDAO propridao=new ProprietaireDAO(); 
			 ArrayList<Proprietaire> prop = propridao.Recherche(rech,nom);
		...
		public Window getFrameListeProprietaire() {
