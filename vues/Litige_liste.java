	...
	public class Litige_liste{
		...
		public static void main(String[] args) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						Litige_liste window = new Litige_liste();
		...
		private void initialize() {
			...
			final JLabel lblNewLabel = new JLabel("Litige liste");
			lblNewLabel.setForeground(Color.WHITE);
			lblNewLabel.setBounds(304, 11, 199, 56);
			...
			cRecherche.setModel(new DefaultComboBoxModel(new String[] {"MOTIF", "NUMERO ", "DATEL", "ID_LOCATAIRE", "ID_CONTRAT"}));
			...
			comboagence_1.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					Litige_liste cat = new Litige_liste();
			...
			lblNewLabel_1.setIcon(new ImageIcon(Litige_liste.class.getResource("/res/law.png")));
			lblNewLabel_1.setBounds(568, 0, 75, 64);
			panel.add(lblNewLabel_1);
			
			JPanel panel_1_1 = new JPanel();
			panel_1_1.setBounds(714, 574, 176, 27);
			panel.add(panel_1_1);
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(table.getSelectedRow()!=-1) {
						 ...
						 if(input==0) {
								...
								litidao.deleteById(selectedId);
								((DefaultTableModel)table.getModel()).removeRow(test);
								JOptionPane.showMessageDialog(null, "Le litige bien été supprimé ");
						 }
			...
			panel_1.setBounds(356, 574, 176, 27);
			panel.add(panel_1);
			panel_1.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if(table.getSelectedRow()!=-1) {
						...
						LitigeDAO litidao=new LitigeDAO();
						Litige p=litidao.getById(selectedId);

								new Litige_edit(p).getFrame().setVisible(true);
			...
			panel_1_2.setBounds(83, 574, 176, 27);
			panel.add(panel_1_2);
			panel_1_2.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					Document doc =new Document();
								...
								PdfWriter.getInstance(doc, new FileOutputStream("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\liti.pdf"));
								doc.open();
								//ENTETE DEBUT
								...
								doc.add(new Paragraph("LITIGE"));
								Image img = Image.getInstance("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\droit.png");
								...
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								doc.add(new Paragraph("Liste des litiges :"));
								doc.add(new Paragraph("---------------------------------------------------------------------------------------------------------------------------------- "));
								...
								cell = new PdfPCell(new Phrase("Motif",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase("DateL",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase("id_Contrat",FontFactory.getFont("Comic Sans MS", 12)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								cell.setBackgroundColor(BaseColor.GRAY);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("motif").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("dateL").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								cell = new PdfPCell(new Phrase (resultat.getString("id_Contrat").toString(),FontFactory.getFont("Arial", 11)));
								cell.setHorizontalAlignment(Element.ALIGN_CENTER);
								table.addCell(cell);
								...
								doc.close();
												...
												try {
													Desktop.getDesktop().open(new File("C:\\Users\\Utilisateur\\Desktop\\eclipse_dossier\\agenceSwing7\\liti.pdf"));
		...
		public static void remp() {
			LitigeDAO litidao = new LitigeDAO();
		     ...
		     String columns[] = {"ID", "MOTIF", "NUMERO ", "DATEL", "ID_LOCATAIRE", "ID_CONTRAT"};
		     String data[][] = new String[liti.size()][columns.length];

		     for(int i=0;i<liti.size();i++) {
		    	 data[i][0] = liti.get(i).getId()+"";
			        data[i][1] = liti.get(i).getMotif();
			        data[i][2] = liti.get(i).getNumero()+"";
			        data[i][3] = liti.get(i).getDateL();
			        data[i][4] = liti.get(i).getId_Locataire()+"";
			        data[i][5] = liti.get(i).getId_Contrat()+"";
		...
		public static void recherche(String rech,String nom) {
			LitigeDAO uproduit = new LitigeDAO();	 
			 ArrayList<Litige> liti = uproduit.Recherche(rech,nom);
		...
		public Window getFrameListeLitige() {
